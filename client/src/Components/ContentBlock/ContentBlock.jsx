...
const ContentBlock = ({myTweets}) => {
  ...
  let currentTweets = search ? selectedTweets : tweets
  if (tweets) {
    if (myTweets) {
      currentTweets = currentTweets.filter(tweet => tweet.user === currentUser)
    }
    currentTweets.sort((prev, next) => next.date - prev.date)
  }
  const receivedTweets = !tweets ? 
    <Loading /> :
